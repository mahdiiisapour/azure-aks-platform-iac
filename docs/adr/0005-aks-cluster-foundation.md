# ADR 0005: AKS Cluster Foundation
...
The platform now has persistent state storage, a persistent development resource group, and a disposable network baseline. The next layer creates a minimal AKS cluster foundation.
...
- One system node only, using `Standard_DC2as_v6`

The user node pool is deferred because of Free Trial quota limits.

## Rationale

AKS Standard features are used rather than AKS Automatic because the project is about platform architecture ownership: identity, networking, node pools, RBAC scopes, and day-2 operations should remain visible.

The lab uses the Free tier because this environment is cost-constrained. Production should use the Standard tier, availability zones, and stronger operational controls.
...
Azure CNI Overlay with Cilium is selected because it avoids consuming VNet IPs for every pod while giving a modern data plane and network-policy path.

The system node pool has one node only because the Free Trial quota cannot support the preferred two or three system nodes. This is not resilient. Production should use at least two system nodes, preferably three or more.
...
AKS Automatic reduces platform decisions and operational surface area. It is not chosen because this repository is intended to expose and document the platform architecture decisions directly.

### AKS Standard tier

The Standard tier is more appropriate for production. It is deferred to control lab cost.
...
The first cluster is intentionally non-resilient. It is useful for validating identity, networking, Azure RBAC, OIDC, Workload Identity, and basic cluster operations, but it should not be treated as production-like.
